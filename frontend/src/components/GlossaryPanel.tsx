        ...
        <div className="w-full max-w-lg max-h-[85vh] bg-[var(--surface)] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-[var(--foreground)]">术语库</h3>
                <p className="text-xs text-[var(--muted)]">{terms.length} 个术语</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-4 space-y-3 border-b border-[var(--border)]">
            <div className="relative">
              ...
              <input
                ...
                placeholder="搜索术语..."
                className="w-full pl-10 pr-4 py-2.5 border border-[var(--border)] rounded-xl text-sm text-[var(--foreground)] bg-[var(--background)] placeholder-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
              />
            </div>
          </div>
                ...
                {filteredTerms.map((term) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-medium text-[var(--foreground)]">{term.english}</span>
                      <span className="text-[var(--muted)]">→</span>
                      <span className="text-blue-600 font-medium">{term.chinese}</span>
                    </div>
                    {term.explanation && (
                      <p className="text-xs text-[var(--muted)]">{term.explanation}</p>
                    )}
                  </div>
                ))}
              </div>
    ...
    <aside className="w-96 h-[calc(100vh-64px)] bg-[var(--surface)] border-l border-[var(--border)] flex flex-col">
      ...
      <div className="p-4 space-y-3 border-b border-[var(--border)]">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-dashed border-[var(--border)] rounded-xl text-sm font-medium text-[var(--muted)] hover:border-blue-400 hover:text-blue-600 hover:bg-blue-500/10 transition-all cursor-pointer"
        >
          <Plus size={18} />
          添加术语
        </button>
        ...
        <div className="p-4 bg-[var(--background)] border-b border-[var(--border)] space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <input
              ...
              placeholder="英文术语"
              className="px-3 py-2 border border-[var(--border)] rounded-lg text-sm text-[var(--foreground)] bg-[var(--surface)] placeholder-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            />
            <input
              type="text"
          ...
          <select
            ...
            className="w-full px-3 py-2 border border-[var(--border)] rounded-lg text-sm text-[var(--foreground)] bg-[var(--surface)] focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 cursor-pointer"
          >
          ...
          <input
            ...
            placeholder="说明 (可选)"
            className="w-full px-3 py-2 border border-[var(--border)] rounded-lg text-sm text-[var(--foreground)] bg-[var(--surface)] placeholder-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
          />
          <div className="flex gap-2">
            <button
              onClick={handleAddTerm}
              disabled={!newTerm.english || !newTerm.chinese}
              className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-[var(--border)] text-white disabled:text-[var(--muted)] rounded-lg text-sm font-medium transition-colors cursor-pointer disabled:cursor-not-allowed"
            >
              <Check size={16} />
              添加
            </button>
            <button
              ...
              className="px-4 py-2 bg-[var(--background)] hover:bg-[var(--border)] text-[var(--foreground)] rounded-lg text-sm font-medium transition-colors cursor-pointer"
            >
              取消
            </button>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="p-4 space-y-2">
            {filteredTerms.map((term) => (
              <div
                key={term.id}
                className="group p-4 bg-[var(--background)] hover:bg-[var(--surface)] border border-transparent hover:border-[var(--border)] rounded-xl transition-all cursor-default"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    {term.explanation && (
                      <p className="text-xs text-[var(--muted)] truncate">{term.explanation}</p>
                    )}
                    <span
                      className="inline-block mt-2 px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500/10 text-blue-600"
                    >
